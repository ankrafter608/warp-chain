...
3. **Фаза 1 — базовый эндпоинт.** `warpscout scan -p awg -exclude-node DME,LED
   -country FI,SE,DE` → полный отчёт рабочих эндпоинтов. В интерактивном
   режиме warp-chain показывает топ-15 списком (пинг, страна, нода) и даёт
   выбрать; Enter или `-plain` — берётся лучший (минимальный пинг).
4. **Фаза 2 — exit-эндпоинт.** `warpscout scan -p awg -through <base>
   -country FI,SE,DE` — тот же выбор из списка. Если скан не удался,
   warp-chain откатывается на `warpscout -best`.
5. **Генерация** конфигов, ссылок и карточки импорта (см. ниже).
...
После каждого скана в интерактивном режиме warp-chain показывает топ-15
рабочих эндпоинтов:

```
  #    ЭНДПОИНТ                PING     ВИДЕН    НОДА    ЛОКАЦИЯ
  1    162.159.192.61:2408     12ms     FI       HEL     Helsinki, Finland
  2    162.159.192.54:2408     15ms     SE       ARN     Stockholm, Sweden
  ...
Эндпоинт фазы 1 (номер, Enter = лучший):
```

Enter — берётся лучший по пингу (первая строка, warpscout сам сортирует);
цифра — выбирается конкретный эндпоинт. Столбец «ВИДЕН» — страна, которую
видят внешние сервисы через туннель. Фазы 1 и 2 выбираются отдельно.
В неинтерактивном режиме (`-plain`, пайп) вопросов нет — всегда берётся
лучший. Список строится из полного отчёта warpscout (все рабочие эндпоинты,
уже отсортированные по пингу); строки секции «torn down» (поднялся и умер
посреди потока) в выбор не попадают.
...
| `-exclude-node` | `DME,LED` | ноды, исключаемые в фазе 1 (дефолт — Домодедово и Пулково, РФ) |
...
| Файл | Что это |
|------|---------|
| `out/warp-chain-profiles.json` | **главный файл для импорта**: оба эндпоинта одним JSON — одна вставка в «Добавить профиль из буфера обмена» создаёт сразу оба профиля (AWG-BASE и WARP-EXIT). warp-chain сам копирует его в буфер (clip.exe / termux-clipboard-set). |
| `out/warp-chain.json` | полный sing-box конфиг (цепочка + TUN + DNS + route) — для прямого запуска ядром. Провалидирован `sing-box check` ядра NekoBox, но как Custom Config в NekoBox не импортируется (см. «Импорт в NekoBox», вариант Б). |
| `out/warp-chain-links.txt` | две share-ссылки `awg://` и `wg://` — запасной способ импорта, по одной вставке на профиль |
...
**Вариант А (два профиля + цепочка) — рабочий способ:**
1. NekoBox: *Сервер → Добавить профиль из буфера обмена*. В буфере уже лежит
   `warp-chain-profiles.json` — warp-chain копирует его сам после генерации
   (clip.exe на Windows, termux-clipboard-set в Termux). Если буфер пуст,
   откройте `out/warp-chain-profiles.json` и скопируйте файл целиком.
   Появятся оба профиля: AWG-BASE и WARP-EXIT.
...
Запасной вариант А — те же два профиля по ссылкам из `out/warp-chain-links.txt`
(по одной вставке на профиль, затем та же цепочка).

**Вариант Б (Custom Config с полным конфигом) — в текущих сборках NekoBox НЕ
работает.** Приложение при запуске само дописывает в каждый эндпоинт поле
`"server": null`, и его ядро отклоняет конфиг: `endpoints[0].server: json:
unknown field "server"`. Это баг NekoBox (проверено на Windows-сборке
5.11.28.3, Android-форк устроен так же), а не конфига: сам `warp-chain.json`
проходит `sing-box check`. Импортированный профиль при этом один и называется
«Обычай» — машинный перевод слова Custom, так и задумано. Файл
`warp-chain.json` остаётся полезным для прямого запуска ядром:
...
Форматы файлов те же; после генерации warp-chain копирует
`warp-chain-profiles.json` в буфер обмена через `termux-clipboard-set`
(нужен пакет `termux-api`) — дальше тот же «Вариант А»: *Сервер → Добавить
профиль из буфера обмена* сразу создаёт оба профиля, вручную из nano ничего
перепечатывать не нужно. Запасной способ — ссылки из `warp-chain-links.txt`:
`wg://` распознаётся, поддержка `awg://` зависит от версии приложения.
Вариант Б (Custom Config с полным конфигом) в NekoBox сейчас не работает —
см. «Импорт в NekoBox».
